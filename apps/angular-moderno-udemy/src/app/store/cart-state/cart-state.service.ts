...
import { CartStorageService } from 'src/app/store/cart-state/cart-storage.service';
...
export class CartStateService {
  ...
  readonly totalAmount = computed(() =>
    this._cartCalculatorService.calculateTotal(this._product())
  );
  readonly productsCount = computed(() =>
    this._cartCalculatorService.calculateItemsCount(this._product())
  );
  ...
  constructor() {
    ...
    if (savedState) {
      this._product.set(savedState.products);
    }
    // Guardar el estado del carrito en el localStorage cada vez que cambie
    // Utilizando effect para reaccionar a los cambios en el carrito (cartStore)
  ...
  removeFromCart(productId: number): void {
    try {
      if (!productId) {
        throw new Error(
          'Product ID is required to remove a product from the cart.'
        );
      }
